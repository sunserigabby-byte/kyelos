...
export default function PhotoPrompt({ dayNum }: Props) {
  ...
  return (
    <div className="bg-gold-light border-2 border-gold rounded-lg p-3 mb-3">
      <button onClick={() => setExpanded((v) => !v)} className="tappable w-full text-left">
        <div className="font-bold text-navy text-sm">📸 Photo Day — Day {dayNum}</div>
        <div className="text-xs text-navy/70 mt-0.5">
          Take front/side/back. Same lighting as last time.
        </div>
      </button>
      {expanded && (
        <div className="mt-3 pt-3 border-t border-gold/40 text-xs text-navy space-y-2">
